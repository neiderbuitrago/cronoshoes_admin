...
Future<int> nuevoCodigostring() async {
  ...
  if (codigos != null) {
    ...
    for (var i = 1; i <= codigos.length; i++) {
      if (listaCodigos.contains(i.toString())) {
        ...
        if (!await ProductosDB.existeElCodigo(i.toString())) {
          break;
        }
      }
    }
  ...
  while (await ProductosDB.existeElCodigo(codigo.toString())) {
    codigo++;
  }
  return codigo;
}
...
class InformeFlotanteInferior extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SnackBar(
        content: informarInferior(
            context: context, titleText: titleText, messageText: messageText));
  }
}
...
//comprobar que el codigo String no este en productos,Multicodigos,Fracciones
